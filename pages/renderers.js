(function () {
  ...
  const metrics = items => `<div class="grid-4 metrics-grid">${items.map(item => `<div class="card"><div class="metric-label">${e(item.label)}${item.icon ? ` <span>${e(item.icon)}</span>` : ''}</div><div class="metric-value">${e(item.value)}</div>${item.badge ? pill(item.badge, item.tone) : `<div class="trend ${e(item.tone)}">${e(item.detail)}</div>`}</div>`).join('')}</div>`;
  ...
  function records(data) {
    const cards = data.records.map(record => { const search = [record.icon, record.title, record.meta, record.value, record.status].join(' ').toLowerCase(); return `<div class="record-card" data-search="${e(search)}"><div class="record-meta"><div class="file-icon">${e(record.icon)}</div><div><h4>${e(record.title)}</h4><p>${e(record.meta)}</p></div></div><div class="record-value"><strong>${e(record.value)}</strong><br>${pill(record.status, record.tone)}</div></div>`; }).join('');
    return `<section id="records" class="page active">${title(data)}${metrics(data.metrics)}<div class="toolbar"><input class="filter" id="recordSearch" placeholder="${e(data.filters.search)}" oninput="filterRecords()">${data.filters.buttons.map(label => `<button class="filter">${e(label)}</button>`).join('')}</div><div id="recordList" class="insight-list">${cards}</div></section>`;
  }
  ...
  function inventory(data) {
    ...
    return `<section id="inventory" class="page active">${title(data)}${metrics(data.metrics)}<div class="table-wrap"><table><thead><tr>${data.columns.map(column => `<th>${e(column)}</th>`).join('')}</tr></thead><tbody>${rows}</tbody></table></div>${panels(data.panels)}</section>`;
  }
